...
var privateKL = base + '/usr/krg/private/';
var publicKL = base + '/usr/krg/public/';
...
module.exports = {
    ...
    prompt.get(usr, function (err, usr) {
      if (err) {
        ...
        openpgp.generateKeyPair(options).then(function (keypair) {
          ...
          fs.writeFile(publicKL + usr.email + '-public.key', pubKey, function (err) {
            if (err) {
              return console.log(err);
            }
          });

          fs.writeFile(privateKL + usr.email + '-private.key', privKey, function (err) {
            if (err) {
              return console.log(err);
            } else {
  ...
  listPublic: function () {
    console.log(list(publicKL));
  },

  listPrivate: function () {
    console.log(list(privateKL));
  },

  exportKeys: function (eml) {
    var pubKeyPath = publicKL + eml + '-public.key';
    var privKeyPath = privateKL + eml + '-private.key';
  ...
  importKey: function (pk) {
    var key_data = fs.readFileSync('./' + pk, 'utf8');
    ...
    prompt.get(['Email'], function (err, result) {
      if (err) {
        ...
        fs.writeFile(publicKL + result.Email + '-public.key', key_data, function (err) {
          if (err) {
            return console.log(err);
          }
        });
